...
def login_page():
    st.image("bbgg.jpg", use_container_width=True)
...
def landing_page():
    ...
    with st.sidebar:
        ...
        #     msg = st.success(f"👋 Welcome, {st.session_state.username}")
        #     time.sleep(2)
        #     msg.empty()
        # else:
        #     st.warning("⚠️ You are not logged in. Please login via the login page.")
        #     st.session_state["page"] = "login"
        #     st.rerun()
            ...
            if st.session_state.active_tab == "viz":
                ...
                if "generated_chart_code" in st.session_state:
                    try:
                        ...
                        # Remove the temporary generated code
                        st.session_state.pop("generated_chart_code", None)
